...
contract Domains  is ERC721URIStorage{
  ...
  constructor(string memory _tld) ERC721("Wassa Name Service", "WNS") payable {
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the wassa name service", "image": "data:image/svg+xml;base64,',
  ...
  // fetches the balance of the contract and sends it to the requester (which has to be the owner!)
  function withdraw() public onlyOwner {
    uint amount = address(this).balance;
    
    // this is where we send the money
